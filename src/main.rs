...
mod set_1 {
    ...
    fn challenge_7() {
        ...
        let mut decrypted = vec![0u8; input_bytes.len()+key.len()];
        decrypter.unwrap().update(&input_bytes[..input_bytes.len()], decrypted.as_mut_slice()).unwrap();

        println!("{}", from_bytes::into_utf8(decrypted).unwrap());
    }
