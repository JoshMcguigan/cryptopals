...
fn challenge_3() {
    ...
    assert_eq!(expected_message, from_bytes::into_utf8(decrypt::single_byte_xor(input_bytes).decrypted_bytes));
}
...
fn challenge_4() {
    ...
    assert_eq!(expected_message, from_bytes::into_utf8(decrypted_message.decrypted_bytes));
}
...
fn challenge_6() {
    ...
    for key_size in 1usize..40 {
        let result = decrypt::multi_byte_xor_for_key_size(input_bytes.clone(), key_size);
...
#[test]
fn challenge_7() {
    let mut file = File::open("resources/set_1/challenge_7.txt").unwrap();
    let mut contents = String::new();
    let _file_read_result = file.read_to_string(&mut contents);
    let input_bytes = into_bytes::from_base64(&contents);
    let key = into_bytes::from_utf8("YELLOW SUBMARINE");

    let decrypted_bytes = decrypt::aes_ecb(input_bytes, key);
